...
\item For each of the $p$ dimensions, use the pseudotime values to model cells' coordinates. This is accomplished with a smoothing spline, producing a set of functions which collectively map $\mathbb{R}_{\geq 0}$ into $\mathbb{R}^p$, producing a smooth curve.
\item Repeat this process until convergence. The total sum of squared distances from cells to their projections onto the curves is used to determine convergence.
\end{enumerate}
...
where $t$ ... \sd{*** Details on how individual lineage curves are obtained and on ``unique pseudotime value''.}
...
This shrinkage step typically ensures that the final curves will form a tree structure with smooth branching events. In both cases (branching or single-lineage), final pseudotime values are derived from the each point’s orthogonal projection onto the curves. Thus, cells belonging to clusters that are included in multiple lineages will have multiple pseudotime values, but these values will agree quite well for cells positioned before the lineage bifurcation.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% DATASETS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Datasets}
We demonstrate the flexibility of Slingshot by applying it to two previously published single-cell RNA-Seq datasets: one with a single, non-branching lineage and one with a lineage bifurcation. The first is a subset of the data used in \citet{Trapnelletal14}, which assayed 271 human skeletal muscle myoblasts (HSMMs) in order to study their development into mature myotubes. In their analysis, they identify a population of contaminating interstitial mesenchymal cells, which we omit from this dataset. This results in a sample of [???] cells believed to form a single, continuous developmental lineage. For our analysis, we use the cluster labels generated by Monocle as well as a set of labels obtained via k-medoids clustering and, as in the original paper, we represent the data in two dimensions obtained by independent component analysis (ICA).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% RESULTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Results}
For our non-branching analysis, we compare several previously proposed lineage inference techniques. Since there is no gold standard for comparison, we comment only on similarities and differences between these methods.
